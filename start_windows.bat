...
timeout /t 2 /nobreak >nul
set "TUNNEL_URL="
for /f "tokens=*" %%a in ('powershell -Command "Select-String -Path 'data\tunnel.log' -Pattern 'https://(?!api\.)[a-zA-Z0-9-]+\.trycloudflare\.com' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value -First 1"') do set TUNNEL_URL=%%a
